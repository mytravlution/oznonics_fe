...
export default function HrDetail({ detailselected, productselected }) {
    // let { id } = useParams();
    // console.log({ id });
    const arr1 = [];
    var sub_category = localStorage.getItem("sub_category")
    if (detailselected === "hiring") {
        arr1.push('Inteview', 'Employee record', 'Shortlisting');
    }
    else if (detailselected === "management") {
        arr1.push('Leaves', 'Acivity Tracking', 'Scheduling');
    }
    else if (detailselected === "statistic") {
        arr1.push('Transportation', 'Culture', 'Learnings');
    }
    ...
    return (
        <div>
            ...
            <div >
                {isPeripheral && <Redirect to={`/addImage/${"hr_"+ext + "_" + subCategorySelected}`} /> }
            </div>
