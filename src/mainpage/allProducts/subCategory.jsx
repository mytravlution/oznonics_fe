...
export default function SubCategory({ subCategorySelected, detailSelected, productselected }) {
    ...
    const onSubCategoryClicked =(event)=> {
        ...
        ":"+event.target.value.split("?")[1]+":::"+event.target.value.split("?")[0]);
        console.log("extention:"+ext+"_"+event.target.value.split("?")[0]);
    ...
    if ((detailSelected === "mechanical") && (subCategorySelected === "product drawings")) {
        ...
        arr1.push('Main Unit?Yes', 'Battery Pack?Yes','Battery Charger?Yes');
    }
    ...
    return (
        <div>
            <div className={isSubCategoryClicked?"prodDetail-outer button-selected":"prodDetail-outer"}>
                ...
                <div>
                    {(isFilethere ==="no" && isSubCategoryClicked) ?
                    <Redirect to={`/addImage/${"products_"+ext+"_"+secondLevel}` }/>
