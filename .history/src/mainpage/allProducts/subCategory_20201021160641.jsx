        ...
        <div>
            <div className="prodDetail-outer">
                ...
                {arr1.map((cat, index) => {
                    return (
                        <div>
                            <button   data-index={index} className={ buttonValue.toString() === index.toString() ? 'clicked-button sub-category-button-color':'sub-category-inner'} value={cat.toLo}  onClick={onSubCategoryClicked}>{cat}</button>
                        </div>
                    )
                })}
            </div>
