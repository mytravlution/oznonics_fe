        ...
        <div>
            <div className="prodDetail-outer">
                ...
                {arr1.map((cat, index) => {
                    return (
                        <div>
                            <button data-index={index} className={buttonValue.toString() === index.toString() ? 'sub-category-format sub-category-button-color' : ' sub-category-format ssub-category-inner'} value={cat.toLowerCase()} onClick={onSubCategoryClicked}>{cat}</button>
                        </div>
                    )
                })}
            </div>
