...
export default function ProductDetail({ detailselecte }) {
    // let { id } = useParams();
    // console.log({ id });
    const arr1 = [];
    ...
    return (
        <div>
            <div className="prodDetail-outer">
                ...
                {arr1.map(cat => {
                    return (
                        <button className="category-inner" value={cat} onClick={onDetailClicked}>{cat}</button>
                    )
                })}
            </div>
            <div >
                {inDetailClicked && <SubCategory subCategorySelected={subCategorySelected} />}
            </div>
