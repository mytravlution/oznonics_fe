        ...
        <div>
            ...
            <div className="second-level-outer">
                ...
                {arr1.map((cat, index) => {
                    return (
                        <div>
                            <button data-index={index} className={buttonValue.toString() === index.toString() ? 'sub-category-format sub-category-clicked' : ' sub-category-format sub-category-unclick'} value={cat.toLowerCase()} onClick={onSecondLeveClick} title="Click to add info">{cat}</button>
                        </div>
                    )
                })}
            </div>
