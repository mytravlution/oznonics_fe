                    ...
                    {arr1.map((cat, index) => {
                        return (
                            <div>
                                ...
                                <button   data-index={index} className={ buttonValue.toString() === index.toString() ? 'clicked-button sub-category-button-color':'detail-button'} value={cat.toLowerCase()}  onClick={onProductClicked}>{cat}</button>

                                {/* <button data-index={index} className={buttonValue.toString() === index.toString() ? 'detail-button-selected' : 'detail-button'} value={cat.toLowerCase} onMouseOver={onProductClicked}>{cat}</button> */}
                            </div>
                        )
                    })}
                </div>
