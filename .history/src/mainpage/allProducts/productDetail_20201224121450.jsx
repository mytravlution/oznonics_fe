...
export default function ProductDetail({ detailselected, productselected }) {
    ...
    return (
        <div>
                ...
                {arr1.map((cat, index) => {
                    if (sub_category === "All") {
                        return (
                            ...
                            className={buttonValue.toString() === index.toString() ? 'category-inner-clicked' : 'category-inner'} value={cat.toLowerCase()} onClick={onDetailClicked}>{cat}</button>
                        )
                    }
            ...
            <div >
                {isPeripheral ? <Redirect to={`/addImage/${ext + "_" + subCategorySelected}`} /> :
                    inDetailClicked &&
