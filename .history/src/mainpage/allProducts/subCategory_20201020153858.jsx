...
export default function SubCategory({ subCategorySelected, detailSelected }) {
    ...
    let btn_class = selected.includes() ? "clicked-button":"sub-category-inner";
    const arr1 = [];
        ...
        <div>
            <div className="prodDetail-outer">
                ...
                {arr1.map((cat, index) => {
                    return (
                        <div>
                            <button className={btn_class} value={cat} onClick={getSelectedItem(cat.id)} key={cat.id}>{cat}</button>
                        </div>
                    )
                })}
            </div>
