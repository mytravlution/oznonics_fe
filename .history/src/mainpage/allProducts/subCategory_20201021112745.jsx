...
export default function SubCategory({ subCategorySelected, detailSelected }) {
    ...
    const [id, setButtonValue] = useState('0');
    ...
    return (
        <div>
            <div className="prodDetail-outer">
                <p>{detailSelected.toUpperCase()}/{subCategorySelected}/{id}</p>

                {arr1.map((cat, index) => {
                    return (
                        <div>
                            <button   data-index={index} className={ (id=== {index}) ? "clicked-button":"sub-category-inner"} value={cat}  onClick={onSubCategoryClicked}>{index}</button>
                        </div>
                    )
                })}
            </div>
