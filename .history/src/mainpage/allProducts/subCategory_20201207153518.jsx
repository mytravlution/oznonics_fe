...
export default function SubCategory({ subCategorySelected, detailSelected, productselected }) {
    ...
    var sub_category = localStorage.getItem("")
            ...
            <div className="prodDetail-outer">
                ...
                {arr1.map((cat, index) => {
                    return (
                        ...
                        <div >
                            <button data-index={index} className={buttonValue.toString() === index.toString() ? 'sub-category-format sub-category-clicked' : ' sub-category-format sub-category-unclick'} value={cat.toLowerCase()} onMouseEnter={onSubCategoryClicked}>{cat}</button>
                        </div>
                    )
                })}
            </div>
                ...
                <div>
                    {isSubCategoryClicked && <SecondLevel subCategorySelected={subCategorySelected} detailSelected={detailSelected} secondLevelSelected={secondLevel} productselected={productselected}/> }
                </div>
        </div>
    )
}
