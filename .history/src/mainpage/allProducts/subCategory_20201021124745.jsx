        ...
        <div>
            <div className="prodDetail-outer">
                ...
                {arr1.map((cat, index) => {
                    return (
                        <div>
                            <button   data-index={index} className={ buttonValue.toString() === index.toString() ? 'clicked-button button-color':'sub-category-inner'} value={cat}  onClick={onSubCategoryClicked}>{cat}</button>
                        </div>
                    )
                })}
            </div>
