        ...
        <div>
            <div className="prodDetail-outer">
                ...
                {arr1.map((cat, index) => {
                    return (
                        <div >
                            <button style={{textAlign:"center"}} data-index={index} className={buttonValue.toString() === index.toString() ? 'sub-category-format sub-category-clicked' : ' sub-category-format sub-category-unclick'} value={cat.toLowerCase()} onClick={onSubCategoryClicked}>{cat}</button>
                        </div>
                    )
                })}
            </div>
                ...
                <div>
                    {isSubCategoryClicked && <SecondLevel subCategorySelected={subCategorySelected} detailSelected={detailSelected} secondLevelSelected={secondLevel} /> }
                </div>
        </div>
    )
}
